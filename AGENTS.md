...
- **ゲートは点でなく帯で足す。** ハブに時代をN個持たせると空の帯がN個できる
  （コロラド追加でひとりぼっちの帯が1→4になった）。新しい帯を開くなら、その帯に
  住人をもう1つ用意する。`npm run gates:map` が孤立と、各ゲートが自分の帯の地形から
  何 Myr ずれているかを出す。**覚えるルールではなく、走らせる検査。**
- **帯は1つの瞬間より広く取る。** 地形は1枚の復元で、数百万年ずれたゲートもそこに立つ
  （ダイナソーパークは70 Ma地形から7.9 Myr）。ぴったり合わせて帯を細かく割ると、
  どのゲートも誰もいない地球に立つことになる。上限の目安は15 Myr。
