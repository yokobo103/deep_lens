...
- **絵に焼き込まれた学名も主張のうち。綴りまで記録と突き合わせる。**
  1文字違いは別の種を指しうる（2026-09-06: Fezouataの絵は `Asaphellus jujanus`、
  記録は `Asaphellus jujuanus`）。差し替えは `src/assets/scenes/<gate-id>.webp` に
  同名で置くだけ。ファイルが無いゲートは絵が出ないだけで壊れない。
