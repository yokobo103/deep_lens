...
地域・分類群・記録環境・古座標はPBDBのCenomanian occurrence記録を静的に正規化した実データです。95 Ma表示は100.5〜93.9 Maの時間窓を代表し、マーカーはPBDB/GPlates古座標の重心です。記録数は個体数ではなく、同時存在や生息域の広がりを直接示すものでもありません。
